...
async function geminiRequest({ model = DEFAULT_MODEL, parts, generationConfig = {} }) {
  ...
  if (!apiKey) throw providerError('GEMINI_API_KEY is not configured.', 503, 'AI_NOT_CONFIGURED');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw providerError(message, response.status >= 500 ? 502 : response.status, 'GEMINI_REQUEST_FAILED');
    }
    return body;
  } catch (error) {
    if (error.name === 'AbortError') throw providerError('AI request timed out.', 504, 'AI_TIMEOUT');
    throw error;
  } finally {
...
async function generateJson({ prompt, imageBase64, mimeType = 'image/jpeg', model = DEFAULT_MODEL }) {
  ...
  return {
    provider: 'gemini',
    model,
    data: parseJsonText(extractText(body)),
...
async function generateImage({ prompt, model = DEFAULT_IMAGE_MODEL }) {
  ...
  if (!image) throw providerError('AI did not return an image.', 502, 'AI_IMAGE_MISSING');
  return { provider: 'gemini', model, image, usage: body.usageMetadata || {} };
}

module.exports = { generateJson, generateImage, providerError };
